...
class ExpectedCharError(Error):
    def __init__(self, pos_start, pos_end, details):
        super().__init__(pos_start, pos_end, 'Expecter Character', details)
...
class RTError(Error):
    ...
    def generate_traceback(self):
        ...
        while context:
            result = f'  file {pos.fn}, line {pos.ln}, in {context.display_name}\n' + result
...
TT_GTE        = 'GTE' # greater that or equal 
TT_COMMA      = 'COMMA' 
TT_ARROW      = 'ARROW'
TT_POW        = 'POW'
TT_NEWLINE    = 'NEWLINE'
    ...
    def make_tokens(self):
        ...
        while self.current_char != None:
            ...
            if self.current_char in ' \t':
                ...
                # s = subtract - or arrow -> for method
                ...
                # will make single equals if one equal and two equals if two tok (aplies simularly to > and < with >=, <=)
...
class ParseResult:
    # takes output from parser and checks for input
    def __init__(self):
        self.error = None
        self.node = None
...
class Parser:
    ...
    def statement(self):
        ...
        if self.current_tok.matches(TT_KEYWORD, 'give'):
            ...
            if not expr:
                self.reverse(res.to_reverse_count)
            return res.sucsess(ReturnNode(expr, pos_start, self.current_tok.pos_start.copy()))
        
        if self.current_tok.matches(TT_KEYWORD, 'continue'):
            res.register_advancement()
            self.advance()
            
            return res.sucsess(ContinueNode(pos_start, self.current_tok.pos_start.copy()))
        
        if self.current_tok.matches(TT_KEYWORD, 'break'):
            res.register_advancement()
            self.advance()
            
            return res.sucsess(BreakNode(pos_start, self.current_tok.pos_start.copy()))
        
        #means it's a normal expr
        expr = res.register(self.expr())
        if res.error: 
            return res.failure(InvalidSyntaxError(self.current_tok.pos_start, self.current_tok.pos_end,
            "Expected: 'give', 'continue', 'break', 'V', 'f', 'four', 'whilst', 'op', int, float, identifier, '+', '-', '(', '[' or 'no'"))
    ...
    def expr(self):
        ...
        if self.current_tok.matches(TT_KEYWORD, 'V'):
            ...
            #check for error for res.register
            if res.error: return res
            return res.sucsess(VarAssignNode(var_name, expr))

        node = res.register(self.bin_op(self.comp_expr, ((TT_KEYWORD, "aswell"), (TT_KEYWORD, "either"))))

        if res.error: 
            return res.failure(InvalidSyntaxError(self.current_tok.pos_start, self.current_tok.pos_end,
            "Expected: 'V', 'f', 'four', 'whilst', 'op', int, float, identifier, '+', '-', '(', '[' or 'no'"))

        return res.sucsess(node)
    ...
    def comp_expr(self):
        ...
        if self.current_tok.matches(TT_KEYWORD, "no"):
            ...
            if res.error: return res
            return res.sucsess(UnaryOpNode(op_tok, node))

        node = res.register(self.bin_op(self.arith_expr, (TT_EE, TT_NE, TT_LT, TT_GT, TT_LTE, TT_GTE)))
        
        if res.error: 
            return res.failure(InvalidSyntaxError(self.current_tok.pos_start, self.current_tok.pos_end,
            "Expected: int, float, identifier, '+', '-', '(', '[', 'f', 'four', 'whilst', 'op' or 'no'"))
    ...
    def call(self):
        ...
        if self.current_tok.type == TT_LPAREN:
            ...
            if self.current_tok.type == TT_RPAREN:
                ...
                res.register_advancement()
                self.advance()
            return res.sucsess(CallNode(atom, arg_nodes))
        return res.sucsess(atom)
    ...
    def atom(self):
        ...
        if tok.type in (TT_STRING):
            res.register_advancement()
            self.advance()
            return res.sucsess(StringNode(tok))

        #check for identifier
        elif tok.type == TT_IDENTIFIER:
            res.register_advancement()
            self.advance()
            return res.sucsess(VarAccessNode(tok))
        ...
        # check for list []
            ...
            if res.error: return res
            return res.sucsess(list_expr)

        # check for if
        elif tok.matches(TT_KEYWORD, 'f'):
            if_expr = res.register(self.if_expr())
            if res.error: return res
            return res.sucsess(if_expr)

        # check for for loop
        elif tok.matches(TT_KEYWORD, 'four'):
            for_expr = res.register(self.for_expr())
            if res.error: return res
            return res.sucsess(for_expr)

        # check for while loop
        elif tok.matches(TT_KEYWORD, 'whilst'):
            while_expr = res.register(self.while_expr())
            if res.error: return res
            return res.sucsess(while_expr)

        # check for function
        elif tok.matches(TT_KEYWORD, 'op'):
            func_def = res.register(self.func_def())
            if res.error: return res
            return res.sucsess(func_def)

        return res.failure(InvalidSyntaxError(tok.pos_start, tok.pos_end, 
        "Expected: Expected: int, float, identifier, '+', '-', '(', [, 'f', 'four', 'whilst', 'op' or 'no'"))
    ...
    def list_expr(self):
        ...
        if self.current_tok.type == TT_RSQUARE:
            ...
            res.register_advancement()
            self.advance()
        return res.sucsess(ListNode(element_nodes, pos_start, self.current_tok.pos_end.copy()))

    def if_expr(self):
        res = ParseResult()
        all_cases = res.register(self.if_expr_cases('f'))
        if res.error: return res
        cases, else_case = all_cases
        return res.sucsess(IfNode(cases, else_case))

    def if_expr_b(self):
        return self.if_expr_cases('ef')
    
    def if_expr_c(self):
        res = ParseResult()
        else_case = None
        
        if self.current_tok.matches(TT_KEYWORD, 'e'):
            res.register_advancement()
            self.advance()
        ...
        return res.sucsess(else_case)

    def if_expr_b_or_c(self):
        res = ParseResult()
        cases, else_case = [], None

        if self.current_tok.matches(TT_KEYWORD, 'ef'):
        ...
        return res.sucsess((cases, else_case))
    ...
    def if_expr_cases(self, case_keyword):
        ...
        if not self.current_tok.matches(TT_KEYWORD, 'after'):
            return res.failure(InvalidSyntaxError(self.current_tok.pos_start, self.current_tok.pos_end, 
            f"Expected: 'after'"))
    ...
    def for_expr(self):
        ...
        if not self.current_tok.matches(TT_KEYWORD, 'four'):
            return res.failure(InvalidSyntaxError(self.current_tok.pos_start, self.current_tok.pos_end, 
            f"Expected: 'four'"))
        ...
        if self.current_tok.type == TT_NEWLINE:
            ...
            return res.sucsess(ForNode(var_name, start_value, end_value, step_value, body, True))

        body = res.register(self.statement())
        if res.error: return res

        return res.sucsess(ForNode(var_name, start_value, end_value, step_value, body, False))                        
    ...
    def while_expr(self):
        ...
        if self.current_tok.type == TT_NEWLINE:
            ...
            return res.sucsess(WhileNode(condition, body, True))

        body = res.register(self.statement())
        if res.error: return res

        return res.sucsess(WhileNode(condition, body, False))  

    def func_def(self):
        res = ParseResult()

        if not self.current_tok.matches(TT_KEYWORD, 'op'):
            return res.failure(InvalidSyntaxError(self.current_tok.pos_start, self.current_tok.pos_end, 
            f"Expected: 'op'"))
        ...
        if self.current_tok.type == TT_ARROW:
            ...
            return res.sucsess(FuncDefNode(var_name_tok, arg_name_toks, node_to_return, True))
        ...
        return res.sucsess(FuncDefNode(var_name_tok, arg_name_toks, node_to_return, False))
...
# for storing and operation numbers
class Number(Value):
    ...
    def subbed_by(self, other):
        if isinstance(other, Number):
            ...
            try:
                ...
                'Elements at this index could not be removed from the list because index is out of bounds',
                self.context)
        else:
    ...
    def dived_by(self, other):
        if isinstance(other, Number):
            ...
            try:
                ...
                return None, RTError(other.pos_start, other.pos_end, 
                'Elements at this index could not be retrieved from the list because index is out of bounds',
                self.context)
        else:
            None, Value.illegal_operation(self, other)
...
class BaseFunction(Value):
    ...
    def check_args(self, arg_names, args):
        ...
        if len(args) > len(arg_names):
            ...
            f"{len(args) - len(arg_names)} too many args passed into '{self.name}'", self.context))
        
        if len(args) < len(arg_names):
            return res.failure(RTError(self.pos_start, self.pos_end,
            f"{len(arg_names) - len(args)} too few args passed into '{self.name}'", self.context))
...
class BuiltInFunction(BaseFunction):
    ...
    def execute(self, args):
        ...
        return_val = res.register(method(exec_ctx))
        if res.should_return():return res
    ...
    def execute_isOperation(self, exec_ctx):
        is_operation = isinstance(exec_ctx.symbol_table.get("value"), BaseFunction)
        return RTResult().success(Number.true if is_operation else Number.false)
    execute_isOperation.arg_names = ['value']    
    ...
    def execute_append(self, exec_ctx):
        ...
        if not isinstance(list_, List):
            ...
            "First argument must be type List", exec_ctx))
            
        list_.elements.append(value)
        return RTResult().success(Number.null)
    execute_append.arg_names = ['list', 'value']
    
    def execute_explode(self, exec_ctx):
        list_ = exec_ctx.symbol_table.get("list")
        ...
        if not isinstance(index, Number):
            return RTResult().failure(RTError(self.pos_start, self.pos_end,
            "Second argument must be type Number", exec_ctx))
        
        try:
            element = list_.elements.pop(index.value)
        except: 
            return RTResult().failure(RTError(self.pos_start, self.pos_end,
            "Element at index could not be transfered because index is out of bounds of list", exec_ctx))
        return RTResult().success(element)
    execute_explode.arg_names = ['list', 'index']

    def execute_extendo(self, exec_ctx):
        ...
        if not isinstance(listB, List):
            return RTResult().failure(RTError(self.pos_start, self.pos_end,
            "Second argument must be type List", exec_ctx))
        
        listA.elements.extend(listB.elements)
        return RTResult().success(Number.null)
    execute_extendo.arg_names = ['listA', 'listB']
    
    def execute_sort(self, exec_ctx):
        list_to_sort = exec_ctx.symbol_table.get("list")
        
        if not isinstance(list_to_sort, List):
            return RTResult().failure(RTError(self.pos_start, self.pos_end,
            "First argument must be type List", exec_ctx))
        
        list_to_sort.elements.sort()
        return RTResult().success(Number.null)
    execute_sort.arg_names = ['list']
    
    def execute_fart(self, exec_ctx):
        print("it is forbiden to fart...")
        return RTResult().success(Number.null)
    execute_fart.arg_names = []
        
    def execute_max(self, exec_ctx):
        valueA = exec_ctx.symbol_table.get("valueA")
        valueB = exec_ctx.symbol_table.get("valueB")
        
        if not isinstance(valueA, Number):
            return RTResult().failure(RTError(self.pos_start, self.pos_end,
            "First argument must be type Number", exec_ctx))
            
        if not isinstance(valueB, Number):
            return RTResult().failure(RTError(self.pos_start, self.pos_end,
            "Second argument must be type Number", exec_ctx))
            
        return RTResult().success(max(valueA, valueB))     
    execute_max.arg_names = ['valueA', 'valueB'] 
    ...
    def execute_min(self, exec_ctx):
        ...
        return RTResult().success(min(valueA, valueB))     
    execute_min.arg_names = ['valueA', 'valueB']   
    ...
    def execute_len(self, exec_ctx):
        ...
        if not isinstance(list_, List):
            return RTResult().failure(RTError(self.pos_start, self.pos_end,
            "Argument must be type List", exec_ctx))
...
BuiltInFunction.isOperation   = BuiltInFunction("isOperation")
BuiltInFunction.append        = BuiltInFunction("append")
BuiltInFunction.explode       = BuiltInFunction("explode")
BuiltInFunction.extendo       = BuiltInFunction("extendo")
BuiltInFunction.sort          = BuiltInFunction("sort")
BuiltInFunction.fart          = BuiltInFunction("fart")
...
class Context:
    def __init__(self, display_name, parrent=None, parent_entry_pos=None):
...
class SymbolTable:
    # to keep track of all the varible names and their data
    ...
    def get(self, name):
        ...
        if value == None and self.parrent:
            return self.parrent.get(name)
        return value
...
class Interpreter:
    ...
    def visit_BinOpNode(self, node, context):
        ...
        if res.should_return(): return res #error check
        ...
        elif node.op_tok.matches(TT_KEYWORD, 'aswell'):
...
global_symbol_table.set("mathPie", Number.mathPi)
...
global_symbol_table.set("isOp", BuiltInFunction.isOperation)
global_symbol_table.set("append", BuiltInFunction.append)
global_symbol_table.set("explode", BuiltInFunction.explode)
global_symbol_table.set("extendo", BuiltInFunction.extendo)
global_symbol_table.set("sort", BuiltInFunction.sort)
global_symbol_table.set("fart", BuiltInFunction.fart)
